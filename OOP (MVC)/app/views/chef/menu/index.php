...
<section class="py-8">
    ...
    <div class="container flex justify-end">
        <a href="<?= URLROOT . '/menus/create' ?>" class="block w-fit mb-6 bg-tertiary px-4 py-1 text-based rounded-lg">Create New Menu <i class="fa-regular fa-plus"></i></a>
        </div>
        <div class="container grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-10 gap-y-4 place-items-center">
        <?php
            foreach ($data as $menu) {
        ?>
            <a href="<?= URLROOT . '/menus/show/' . htmlspecialchars($menu["id"]) ?>" class="bg-tertiary rounded-lg pb-3 px-2 pt-2 max-w-80">
        ...
        <a href="<?= URLROOT . '/menus/create' ?>" class="block w-fit mt-10 mx-auto bg-tertiary px-4 py-1 text-based rounded-lg">Create New Menu <i class="fa-regular fa-plus"></i></a>
</section>
