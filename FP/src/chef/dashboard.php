<?php
    ...
    $pendingReservationsCount = getReservationsCountWithStatus("pending");
    $todayConfirmedReservationsCount = getConfirmedReservationsCountOfDate(date('Y-m-d'));
    $tommorowConfirmedReservationsCount = getConfirmedReservationsCountOfDate(date('Y-m-d', strtotime('+1 day')));
    $nextReservation = getNextConfirmedReservation();
    $reservationsRecievedInLastWeek = getLastWeekReservationsCountGrouped();
    $clientsCount = getUsersCountByRole("client");
?>
        ...
        <div>
            ...
            <div class="w-fit items-center bg-white shadow-lg rounded-lg p-6 flex flex-col gap-6">
                ...
                <div class="flex-shrink-0 mt-4 md:mt-0">
                    <a href=<?= "/show-reservation?id=". $nextReservation["id"] ?> class="bg-tertiary text-white font-semibold py-1 px-4 rounded-lg shadow">
                        View Details
                    </a>
                </div>
            </div>
