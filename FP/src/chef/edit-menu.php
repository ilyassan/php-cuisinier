<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){           
        ...
        if(empty($errors['name_err']) && empty($errors['price_err'])){
            try{
                updateMenu($id, $menu["name"], $menu["price"]);
                removeAttachedDishesFromMenu($id);
                attachDishesToMenu($id, $dishesIds);

                flash('success', 'The menu has been updated successfully.');
                redirect('show-menu', $id);
            }catch(Exception $e){
    ...
    $menuWithDishes = getMenuWithDishes($id);

    if (!$menuWithDishes) {
        redirect("menus");
    }

    $menu = $menuWithDishes["menu"];
    $menuDishes = $menuWithDishes["dishes"];
    $dishes = getAllDishes();
?>
